...
export class BamlAsyncClient {
  ...
  async ExtractEntitySentiments(
    ...
    try {
      ...
      return raw.parsed(false) as EntitySentiment[]
    } catch (error) {
  ...
  async ExtractName(
    ...
    try {
      ...
      return raw.parsed(false) as NameInformation
    } catch (error) {
      throw toBamlError(error);
    }
  }
...
class BamlStreamClient {
  ...
  ): BamlStream<(partial_types.EntitySentiment | null)[], EntitySentiment[]> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector ? (Array.isArray(options.collector) ? options.collector : [options.collector]) : [];
      const env = options.env ? { ...process.env, ...options.env } : { ...process.env };
      ...
      return new BamlStream<(partial_types.EntitySentiment | null)[], EntitySentiment[]>(
        raw,
        (a): (partial_types.EntitySentiment | null)[] => a,
        (a): EntitySentiment[] => a,
        this.ctxManager.cloneContext(),
      )
    } catch (error) {
  ...
  ExtractName(
      message: string,
      __baml_options__?: { tb?: TypeBuilder, clientRegistry?: ClientRegistry, collector?: Collector | Collector[], env?: Record<string, string | undefined> }
  ): BamlStream<partial_types.NameInformation, NameInformation> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector ? (Array.isArray(options.collector) ? options.collector : [options.collector]) : [];
      const env = options.env ? { ...process.env, ...options.env } : { ...process.env };
      ...
      return new BamlStream<partial_types.NameInformation, NameInformation>(
        raw,
        (a): partial_types.NameInformation => a,
        (a): NameInformation => a,
        this.ctxManager.cloneContext(),
      )
    } catch (error) {
